...
if page == "Dashboard":
    ...
    # Display as markdown table to avoid pyarrow
    last_10 = df.tail(10).copy()
    last_10.index = last_10.index.strftime('%Y-%m-%d')
    
    # Create markdown table
    markdown_table = "| Date | Open | High | Low | Close | Volume |\n"
    markdown_table += "|------|------|------|-----|-------|--------|\n"
    
    for idx, row in last_10.iterrows():
        markdown_table += f"| {idx} | ${row['Open']:.2f} | ${row['High']:.2f} | ${row['Low']:.2f} | ${row['Close']:.2f} | {int(row['Volume']):,} |\n"
    
    st.markdown(markdown_table)
    
    # CSV download button (this doesn't need pyarrow)
    csv = df.to_csv()
