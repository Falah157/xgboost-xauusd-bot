...
st.set_page_config(page_title="PHASE 1 - XAUUSD & BTCUSD", layout="wide", page_icon="🏆")

st.markdown('<h1 style="text-align: center;">🏆 PHASE 1 TRADING DASHBOARD</h1>', unsafe_allow_html=True)
...
SYMBOLS = {
    "XAUUSD": {"api": "XAU/USD", "name": "Gold", "digits": 2, "color": "#ffd700", "spread": 0.50},
    "BTCUSD": {"api": "BTC/USD", "name": "Bitcoin", "digits": 0, "color": "#ff8c00", "spread": 15.0}
}
...
def get_market_snapshot(df, current_price):
    """Get 24h market snapshot"""
...
def get_asian_range(df):
    """Get Asian session high/low (12am-6am GMT)"""
...
def get_open_candle_direction(df):
    """Get first 15min candle direction"""
    if df is not None and len(df) > 0:
        first = df.iloc[0]
...
with st.sidebar:
    ...
    refresh_interval = st.slider("Auto Refresh (seconds)", 30, 120, 60)
    ...
    if st.session_state.backtest_results:
        ...
        st.metric("Backtest Win Rate", f"{res['win_rate']:.1f}%")
...
if df is not None and len(df) > 30:
    ...
    # 1. MARKET SNAPSHOT
    snapshot = get_market_snapshot(df, current_price)
    if snapshot:
        st.subheader("📊 24H MARKET SNAPSHOT")
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Open", f"${snapshot['open']:.2f}")
        c2.metric("High", f"${snapshot['high']:.2f}")
        c3.metric("Low", f"${snapshot['low']:.2f}")
        c4.metric("24H Change", f"{snapshot['change']:+.2f}%")
    
    # 2. ASIAN SESSION RANGE
    asian_high, asian_low = get_asian_range(df)
    if asian_high and asian_low:
        st.subheader("🌏 ASIAN SESSION (12am-6am GMT)")
        c1, c2 = st.columns(2)
        c1.metric("Asian High", f"${asian_high:.2f}")
        c2.metric("Asian Low", f"${asian_low:.2f}")
    
    # 3. FIRST 15min CANDLE DIRECTION
    direction, color = get_open_candle_direction(df)
    st.info(f"📊 FIRST 15min CANDLE: **{direction}**")
    ...
    col4.metric("Auto Refresh", f"{refresh_interval}s")
...
st.caption("🏆 PHASE 1: Auto Refresh | Market Snapshot | Asian Range | Open Candle")
