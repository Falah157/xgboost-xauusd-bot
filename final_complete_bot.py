...
st.set_page_config(page_title="FULL AUTO TRADING BOT", layout="wide", page_icon="🏆", initial_sidebar_state="expanded")
...
<style>
    ...
    .timeframe-card { background: #1e1e2e; border-radius: 10px; padding: 0.5rem; text-align: center; border: 1px solid #333; }
    .timeframe-active { border-left: 4px solid #00ff88; }
</style>
""", unsafe_allow_html=True)

st.markdown('<div class="main-header">🏆 FULL AUTO XAUUSD TRADING BOT</div>', unsafe_allow_html=True)
...
TIMEFRAMES_CONFIG = {
    "1m": {"api": "1min", "hours": 1/60, "days": 1, "send_interval": 1, "color": "#ff4444", "pred_bars": 4},
    "5m": {"api": "5min", "hours": 5/60, "days": 3, "send_interval": 5, "color": "#ff8844", "pred_bars": 4},
    "15m": {"api": "15min", "hours": 15/60, "days": 7, "send_interval": 15, "color": "#ffcc44", "pred_bars": 4},
    "30m": {"api": "30min", "hours": 30/60, "days": 14, "send_interval": 30, "color": "#ffff44", "pred_bars": 4},
    "1h": {"api": "1h", "hours": 1, "days": 30, "send_interval": 60, "color": "#88ff44", "pred_bars": 4},
    "4h": {"api": "4h", "hours": 4, "days": 60, "send_interval": 240, "color": "#44ff88", "pred_bars": 2},
    "1d": {"api": "1day", "hours": 24, "days": 180, "send_interval": 1440, "color": "#44ffff", "pred_bars": 1}
}
...
with st.sidebar:
            ...
            if updates and 'result' in updates and updates['result']:
                for update in updates['result']:
                    if 'message' in update and 'chat' in update['message']:
                        ...
                        send_telegram_message(chat_id, "✅ Full Auto Trading Bot Connected!")
    ...
    # Auto Signals - EACH TIMEFRAME INDEPENDENTLY
    st.subheader("🚀 AUTO SIGNALS (Each Timeframe)")
    ...
    # Show each timeframe with its interval
    active_timeframes = {}
...
# ============ AUTO SIGNAL LOOP ============
# This runs EACH TIMEFRAME at its OWN INTERVAL
if st.session_state.auto_run and st.session_state.telegram_connected:
    ...
    for tf in ["1m", "5m", "15m", "30m", "1h", "4h", "1d"]:
        ...
        # Calculate minutes since last signal for this timeframe
        if last_sent is None:
            minutes_since = interval + 1  # Force send if never sent
        else:
            minutes_since = (current_time - last_sent).total_seconds() / 60
        
        # Send signal if interval has passed
        if minutes_since >= interval:
            st.info(f"🔄 Analyzing {tf}... (every {interval}min)")
            ...
            if signal:
                ...
                if send_telegram_message(st.session_state.telegram_chat_id, message):
                    ...
                    st.success(f"✅ {tf.upper()} signal sent to Telegram! (Confidence: {signal['confidence']:.1%})")
                    
                    # Also show in dashboard
                    with st.container():
                        color = "#00ff88" if signal['direction'] == "LONG" else "#ff4444"
                        st.markdown(f"""
                        <div style="background: #1e1e2e; border-left: 4px solid {color}; padding: 0.5rem; margin: 0.2rem 0; border-radius: 5px;">
                            <b>🎯 {tf.upper()} SIGNAL</b> | {signal['direction']} @ ${signal['entry']:.2f} | Confidence: {signal['confidence']:.1%}
                        </div>
                        """, unsafe_allow_html=True)
            else:
                # Still update last check time to avoid spamming API
                st.session_state.last_signal_sent[tf] = current_time
    ...
    with st.spinner(f"Loading {current_timeframe} data..."):
        ...
        if df is not None and len(df) > 20:
            ...
            if prediction == 1 and confidence >= min_conf:
                st.markdown(f'<div class="signal-bullish"><h2 style="color: #00ff88;">📈 BULLISH SIGNAL</h2><p>Confidence: {confidence:.1%}</p></div>', unsafe_allow_html=True)
                direction = "LONG"
            elif prediction == 0 and confidence >= min_conf:
                st.markdown(f'<div class="signal-bearish"><h2 style="color: #ff4444;">📉 BEARISH SIGNAL</h2><p>Confidence: {confidence:.1%}</p></div>', unsafe_allow_html=True)
                direction = "SHORT"
            else:
...
with tab3:
    ...
    if st.session_state.all_signals:
        ...
        col1, col2 = st.columns(2)
        with col1:
            filter_tf = st.selectbox("Filter by Timeframe", ["All"] + list(TIMEFRAMES_CONFIG.keys()))
        with col2:
        ...
        st.info("No signals recorded yet. Enable Auto-Signals to receive signals automatically!")
...
with tab5:
    ...
    if st.session_state.all_signals:
        ...
        col4.metric("Active Timeframes", len(st.session_state.last_signal_sent))
        ...
        # Last signal per timeframe
        st.subheader("⏱️ Last Signal by Timeframe")
        for tf in TIMEFRAMES_CONFIG.keys():
            last = st.session_state.last_signal_sent.get(tf)
            if last:
                st.text(f"{tf}: {last.strftime('%Y-%m-%d %H:%M:%S')}")
            else:
                st.text(f"{tf}: Never")
    else:
        st.info("No data yet. Enable Auto-Signals to start receiving signals!")
...
col3.metric("Total Signals", len(st.session_state.all_signals))

st.caption("⚠️ EDUCATIONAL ONLY - Not financial advice. Each timeframe sends signals at its own interval (1m=1min, 5m=5min, 15m=15min, etc.)")
